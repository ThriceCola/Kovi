...
pub type AdminMsgEvent = MilkyEvent<AdminMessageReceiveEventData>;

impl MessageEventTrait for AdminMsgEvent {
...
impl TryFrom<MsgEvent> for AdminMsgEvent {
    ...
    fn try_from(event: MsgEvent) -> Result<Self, Self::Error> {
        ...
        Ok(AdminMsgEvent {
            ...
            data: AdminMessageReceiveEventData {
                time: data.time,
...
impl AdminMsgEvent {
    pub(crate) fn new(
        ...
        if !bot_info.any_admins_contains(RefID::new(&event.data.sender_id)) {
            return Err(EventBuildError::ParseError(
                "message sender is not an admin".to_string(),
            ));
        }

        let event = AdminMsgEvent::try_from(event)?;
...
impl MessageEventUtil for AdminMsgEvent {
