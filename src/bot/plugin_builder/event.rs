...
use crate::{Bot, types::ApiAndOneshot};
...
impl std::hash::Hash for EventType {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            EventType::Message => 0.hash(state),
            EventType::Notice => 1.hash(state),
            EventType::Request => 2.hash(state),
            EventType::MetaEvent => 3.hash(state),
        }
    }
}

pub trait Event: Any + Send + Sync {
    fn de(json_str: &str, bot: &Bot, api_tx: mpsc::Sender<ApiAndOneshot>) -> Option<Self>
    where
        Self: Sized;
}
